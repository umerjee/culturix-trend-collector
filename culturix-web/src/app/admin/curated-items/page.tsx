...
type Plan = { duration_seconds: number; beat_count: number; rationale: string; allowed_durations: number[]; source_chars: number; thin_source: boolean; existing_draft: boolean; estimate: Record<string, { gpu_seconds: number; cost_usd: number }> };
...
export default function CuratedItemsPage() {
  ...
  async function openPlan(item: Item) {
    setPlanItem(item); setPlan(null); setPlanError(""); setUseHost(false);
    try {
    ...
    {planItem && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label="Plan World video">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div className="mt-6 flex justify-end gap-2">
          <button onClick={() => setPlanItem(null)} className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-600">Cancel</button>
          <button disabled={!plan || generating || plan.existing_draft} onClick={confirmGenerate} className="rounded-lg bg-primary-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{generating ? "Starting..." : `Write ${duration}s script`}</button>
        </div>
      </div>
    </div>}
  </div>;
