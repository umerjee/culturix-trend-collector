        ...
        <nav className="flex items-center gap-1">
          {ITEMS.map(({ key, href, icon, label }) => (
            <Link
              key={key}
              href={href}
              className={`inline-flex items-center gap-1.5 text-sm font-medium rounded-lg px-3 py-2 transition-colors ${
                active === key
          ...
          {isSuperAdmin && (
            <Link
              href="/admin"
              className="inline-flex items-center gap-1.5 text-sm font-medium text-indigo-600 border border-indigo-200 rounded-lg px-3 py-2 hover:bg-indigo-50 transition-colors ml-1"
            >
          ...
          <form action="/api/auth/signout" method="POST" className="ml-1">
            <button
              type="submit"
              title="Sign out"
              className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-400 rounded-lg px-3 py-2 hover:bg-gray-50 hover:text-gray-600 transition-colors"
            >
