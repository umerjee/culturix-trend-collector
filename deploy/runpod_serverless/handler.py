...
Input: `{"workflow": <ComfyUI API-format JSON>}`, one LTX-2.5 generation
per job — a whole multi-shot segment rendered in one call (2.5's own
native multishot), not one job per shot. Optionally `reference_image_base64`
(single-anchor mode) or `reference_images_base64` (MSR multi-reference
mode, mutually exclusive — see app/media/ltx25_workflow.py).

(A prior version also accepted `shot_workflows` for one-LTX-generation-
PER-SHOT jobs, and narration_audio_base64/narration_text for worker-side
ElevenLabs muxing / on-GPU Chatterbox synthesis — both were for the
LTX-2.3 path, which rendered shot-by-shot and generated audio separately.
Removed 2026-09-17 along with LTX-2.3 and per-character LoRA training:
LTX-2.5 renders a whole segment per job with audio and video denoised
jointly in one pass, so nothing calls this worker with either shape
anymore.)
"""
import base64
import logging
...
def _ensure_faststart(video_bytes: bytes) -> tuple:
    ...
    concat, which also writes a fresh container that would otherwise
    regress to moov-after-mdat), so every caller of this endpoint gets a
    playable file rather than needing to know to remux it themselves.
...
def handler(event: dict) -> dict:
    ...
    try:
        ...
        # Applied here, on the final video, since ComfyUI's SaveVideo output
        # (see _ensure_faststart's own docstring) needs it regardless.
