"""Self-hosted (RunPod Serverless + ComfyUI + LTX-2.5) counterpart to
app/services/culturetoon_video.py's Kling Omni path. Renders a script's
segments via generate_toon_video_ltx25 — LTX-2.5 generates a whole
multi-shot segment in ONE generation with native synchronized audio and
no per-character LoRA training; identity comes from image conditioning
(a composite/backdrop anchor, or MSR's independent multi-reference
conditioning for segments with more than one present cast member — see
app/media/ltx25_workflow.py's LTX25_MSR_ENABLED). The earlier LTX-2.3
per-shot/per-character-LoRA path (and per-character LoRA training
entirely) has been removed — see git history if it's ever needed again.
...
generate_toon_video). No QA run here yet (unlike the Kling path) — out of
scope for wiring the button; QA parity for self-hosted can follow
separately.
"""
import logging
import os
...
def generate_video_for_toon_selfhosted(user_id, toon_id) -> None:
    ...
    generate_toon_video_ltx25() which just returns bytes — every call
    here is a single ad-hoc click, not a batch window."""
