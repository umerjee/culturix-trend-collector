...
**Phase 1 — Foundation (schema only, no new UI).** `Character.personality` JSONB column. `character_relationships` table (character-level, per decision 5). `generation_usage` table + wrapper around the three existing providers (`KlingOmniProvider`, `HybridImageProvider`, `ElevenLabsProvider`) to record every call — no budget enforcement yet, just visibility. This phase alone closes the single highest-risk gap (§1, live financial exposure) with the least code.

**Phase 2 — Budget enforcement.** Wire `generation_usage` into an actual gate (daily/monthly budget per `CharacterBrand`, warning + hard-stop), reusing the existing `app/billing.py` pattern.

**Phase 3 — Relationships + personality, wired into generation.** `/dashboard/culturetoons` Characters tab gains a Relationships sub-view (simple list, not a graph — a graph is a nice-to-have, not a blocker) and a personality editor (sliders + behavioral-rule list per §5's structured shape). `culturetoon_script.py`'s prompt builder is rewritten to actually consume `Character.personality` and relationship context when casting 2+ characters together — this is the phase where "deterministic character identity" (decision 7) becomes real, not just a schema addition.

**Phase 4 — Character memory (manual creation first).** `character_memories` table, a simple "Add memory" form on the character detail view, Qdrant retrieval (new collection, reusing the existing Voyage.ai + Qdrant infrastructure) wired into script generation. Automated memory extraction from published episodes deferred until there's evidence users want it.

**Phase 5 — Culture library.** `cultures` table, seeded with the cultures already in active use (Indian, Chinese, African, per this session's real character roster), `CharacterVariant.culture_tag` becomes FK-with-fallback, prompt builders (character image, background image, script) all pull structured culture context instead of a bare tag — directly targets the ethnicity-rendering and background-accuracy bugs already found this session.

**Phase 6 — Provider protocols.** Extract `VideoProvider`/`VoiceProvider` protocols (§3.8), adapt `KlingOmniProvider`/`ElevenLabsProvider` to implement them. No new provider yet — this phase is purely about the seam existing before it's needed.

**Phase 7a — QA: technical + visual (deterministic).** Duration, aspect ratio, file integrity, audio-presence checks, run automatically after generation. New `qa` status inserted into the `Toon` status flow (`ready → qa → posted`).

**Phase 7b — QA: AI judge (comedy/cultural/story).** LLM-judge scoring (Qwen-max/Haiku, same pattern as everywhere else), grounded in `cultures.stereotypes_to_avoid` from Phase 5. Returns the full `{visual_score, comedy_score, cultural_score, technical_score, overall_score, publish_recommended}` shape; `publish_recommended: false` soft-blocks the publish button with a visible reason, never auto-archives. Landed as its own PR immediately behind 7a, not gated on a waiting period (decision 6).

**Phase 8 — Analytics feedback loop.** Scheduled aggregation job (existing `app/scheduler.py` — no new Railway Cron Service), `ToonPost` performance summarized by `(character combination, culture, tone, duration bucket)` and injected into script-suggestion prompts.

**Not on the active roadmap:**
- **Scene-level generation / ffmpeg assembly** — decided against (decision 2). Not built unless a future decision explicitly reopens this.
- **Real Workspace/multi-tenancy** — decided against for now (decision 1).
- **LangGraph production pipeline** — decided against (decision 4).
- **Worker/queue hardening** (startup reconciliation pass, or a real Redis/RQ queue) — deliberately **not scheduled as a phase**; tracked as a pre-launch hardening backlog item per decision 3, to be picked up once the product is otherwise ready to go live, not before.

Each phase lands as its own small PR/commit, tested against the existing `tests/test_culturetoons.py` suite (89+ tests today) plus new tests for whatever that phase adds — matching this repo's own established testing discipline, not a new convention.
